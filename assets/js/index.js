      ...
      .map(
        ...
        <article class="rounded-xl border border-slate-800 bg-slate-900/70 p-5">
          ...
          <h3 class="mt-2 text-lg font-semibold text-slate-100">
            <a class="hover:text-cyan-300" href="./post.html?slug=${encodeURIComponent(post.slug)}">${stripHtmlTags(post.title)}</a>
          </h3>
          <p class="mt-2 text-sm text-slate-300">${stripHtmlTags(post.excerpt)}</p>
